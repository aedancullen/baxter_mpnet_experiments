...
class MLP(nn.Module):
	def __init__(self, input_size, output_size):
		...
		self.fc = nn.Sequential(
                    nn.Linear(input_size, 1280), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(1280, 896), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(896, 512), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(512, 384), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(384, 256), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(256, 128), nn.PReLU(), nn.Dropout(p=0.001),
                    nn.Linear(128, 64), nn.PReLU(), nn.Dropout(p=0.001),
